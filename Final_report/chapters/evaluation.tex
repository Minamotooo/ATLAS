...
edges per skill, and the fraction of skills with no prerequisite edge at all.

\textbf{Findings.} The graph averaged fewer than four skills per topic and 1.1
edges per skill, with a substantial fraction of skills having no incoming edge.
A gate that has no parents to test degenerates to the ungated case, and a topic
holding three skills cannot localise a failure within itself --- which is the
system's entire purpose.
...
current tree. \Cref{tab:smoke} lists what each check establishes.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{>{\hsize=0.42\hsize}Y>{\hsize=0.82\hsize}Y>{\hsize=1.76\hsize}Y}
    ...
    \midrule
    Ontology & Graph builds (430 skills); catalogue topics all resolve (66); no legacy topic codes remain & The served ontology is internally consistent and the catalogue cannot reference a topic that does not exist \\
    ...
    Diagnostic & Session starts at length 30; question carries its subject; topic shown as label not code; all 30 questions answered & The adaptive loop runs to completion and returns learner-presentable metadata (FR3, FR12) \\
    Mastery & Mastery unlocked after diagnostic; written to \texttt{user\_skill} (43 skills above zero); section state reports subject & Evidence from 30 questions propagates to far more than 30 skills --- the propagation mechanism doing its intended work (FR4, FR5, FR7) \\
    ...
    Mastery view & View unlocked; graph returns 53 nodes and 52 edges; table rows carry subject & The visualisation contract holds (FR10) \\
    Practice & Topic practice serves questions (8 answered) & Post-diagnostic flow works (FR8) \\
    Spillover & Fires on repeated wrong answers, reporting rule B & The prerequisite-detection policy triggers on the intended evidence (FR9) \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{>{\hsize=0.3\hsize}Y>{\hsize=0.45\hsize}Y>{\hsize=2.25\hsize}Y}
    ...
    FR5 & Met & Ancestor invariant verified explicitly; 30 answers moved 43 skills \\
    ...
    FR10 & Met & Mastery graph (53 nodes, 52 edges) and table both returned \\
...
O1--O5, O7 and O8 are met as evidenced above. \textbf{O6 (question bank) is
partially met}: the pipeline is built, validated and has produced and loaded
real verified items, but full coverage of the specification space was not
completed within the project schedule. \textbf{O3 is met in implementation but
unvalidated in effect} --- the propagation is provably consistent, but whether
the resulting estimates correspond to real learner knowledge is exactly the
question the next subsection says was not answered.
...
\begin{itemize}
  ...
  \item \textbf{Question bank coverage is partial}, so a full-coverage session is
    not yet reachable for every topic.
\end{itemize}
...
\subsection{The rebuilt ontology is not yet the served ontology}

\textbf{Deviation.} The application serves the 430-skill legacy ontology; the
validated 1{,}688-skill rebuild is complete but not adopted.

\textbf{Analysis.} Adoption is not merely a file swap. The question bank is
keyed to legacy skill identifiers, so adopting the new graph without
regenerating content would leave most new skills with no questions --- a system
that knows about 1{,}688 skills but can ask about few of them is worse for a
learner than one that knows about 430 and can ask about most.

\textbf{Decision.} Deliberately sequenced rather than rushed: adopt after a
budgeted generation run against the new specification space
(\Cref{sec:budget}). The dependency is recorded so the sequence is not
