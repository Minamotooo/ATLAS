...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{>{\hsize=0.5\hsize}Y>{\hsize=0.6\hsize}Y>{\hsize=1.9\hsize}Y}
    ...
    \midrule
    Sayaad Muzahid Masfi (2105066) & Adaptive engine & BKT implementation, the three-phase mastery policy, the skill-graph data structure with its cycle check, and the policy specification document that the code is audited against. \\
    Aurchi Chowdhury (2105083) & Backend API \& sessions & FastAPI service, session lifecycle for diagnostic and practice, the tiered question search, and the spillover rules. \\
    Omar Abdur Razzaque (2105094) & Content pipeline & Corpus parsing and repair, retrieval index, the generate-then-verify generation pipeline, and the database loader with its validation pass. \\
    Sakif Naieb Raiyan (2105065) & Frontend & React client, the question-answer flow, mastery visualisation, bilingual interface, and mathematical rendering. \\
    Rubaiyat -E- Zaman (I2105104) & Ontology & Skill extraction from the corpus, the ontology compiler, the full-corpus rebuild and its editorial pass, and the structural validators. \\
    \bottomrule
  \end{tabularx}
\end{table}
