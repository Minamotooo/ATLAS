...
\begin{longtable}{@{}p{11mm}p{56mm}p{\dimexpr\textwidth-11mm-56mm-4\tabcolsep\relax}@{}}
  ...
  \endlastfoot
    ...
    Granularity conflicts with question-bank coverage: finer skills localise gaps better but multiply the items needed (the rebuilt ontology raised the generation space from $430\times6$ to $1{,}688\times6$ tuples). Diagnostic length conflicts with learner patience: a diagnostic short enough to be sat through cannot directly measure 1{,}688 skills, forcing inference by propagation rather than measurement. Adaptivity conflicts with stability: an aggressive learning rate tracks change quickly but overreacts to a lucky guess. \Cref{sec:alternatives} resolves these explicitly. \\
