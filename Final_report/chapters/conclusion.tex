...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{>{\hsize=0.22\hsize}Y>{\hsize=0.45\hsize}Y>{\hsize=2.33\hsize}Y}
    ...
    Obj. & Status & Basis \\
    \midrule
    O1 & Achieved & 1{,}688 skills, 1{,}743 edges, all 183 syllabus topics populated; validated acyclic, depth 9, zero isolated skills, zero unresolved references; adopted as the served catalogue \\
    ...
    O4 & Achieved & The diagnostic sizes itself to the section under test, runs to completion, and releases the mastery gate; the ancestor invariant holds afterwards \\
    O5 & Achieved & Topic practice runs to the mastery threshold; spillover fires on the intended evidence \\
    O6 & \textbf{Partially achieved} & Pipeline built, validated, and producing verified items; coverage incomplete, and reset by adoption --- the $1{,}688 \times 6 = 10{,}128$ specification space is largely ungenerated because existing items are keyed to legacy skill identifiers \\
...
\begin{enumerate}
  ...
  \item \textbf{Question-bank coverage is the binding limitation.} The rebuilt
    ontology has been adopted as the served catalogue, which is the right
    outcome structurally but converts the project's structural deficit into a
    coverage one: existing items are keyed to legacy skill identifiers, so the
    $10{,}128$-specification space against the served ontology is largely
    ungenerated. The system can now diagnose more finely than it can ask.
...
\textbf{Completing the knowledge base --- the largest outstanding item.} The
rebuilt ontology is adopted and the generator has been repointed at it; what
remains is the budgeted generation run itself across the
$1{,}688 \times 6 = 10{,}128$ specification space, prioritised by the topics a
learner reaches first rather than run uniformly. Alongside it, domain-expert
review of the 32 disconnected fragments and the hand-authored edges, since a
wrong prerequisite edge propagates silently under \Cref{eq:pullup}.
